<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="区块链账户私钥" prop="privateKey" width="120" />
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="区块链账户私钥:"  prop="privateKey" >
...
import {
  ...
  deleteStorageContract,
  deleteStorageContractByIds,
...
const getTableData = async() => {
  const table = await getStorageContractList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteStorageContractByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateStorageContractFunc = async(row) => {
    const res = await findStorageContract({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.resc
...
// 删除行
const deleteStorageContractFunc = async (row) => {
    const res = await deleteStorageContract({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
             ...
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  ...
                  break
                case 'update':
                  res = await updateStorageContract(formData.value)
                  break
                default:
                  res = await createStorageContract(formData.value)
                  break
