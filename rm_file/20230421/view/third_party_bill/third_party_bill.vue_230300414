  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateThirdPartyBillFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createThirdPartyBill,
  deleteThirdPartyBill,
  deleteThirdPartyBillByIds,
...
const getTableData = async() => {
  const table = await getThirdPartyBillList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteThirdPartyBillByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateThirdPartyBillFunc = async(row) => {
    const res = await findThirdPartyBill({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.retpb
...
// 删除行
const deleteThirdPartyBillFunc = async (row) => {
    const res = await deleteThirdPartyBill({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createThirdPartyBill(formData.value)
                  break
                case 'update':
                  res = await updateThirdPartyBill(formData.value)
                  break
                default:
                  res = await createThirdPartyBill(formData.value)
                  break
