  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
           ...
           <el-form-item label="交易员工ID" prop="tradingUserId">
            <el-select v-model="searchInfo.tradingUserId" clearable placeholder="请选择" @clear="()=>{searchInfo.tradingUserId=undefined}">
        ...
        <el-form-item label="账单所属机构">
         <el-input v-model="searchInfo.organization" placeholder="搜索条件" />
           ...
           <el-form-item label="对账员ID" prop="reconciler">
            <el-select v-model="searchInfo.reconciler" clearable placeholder="请选择" @clear="()=>{searchInfo.reconciler=undefined}">
              <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
            </el-select>
            </el-form-item>
           <el-form-item label="审核员ID" prop="reviewer">
            <el-select v-model="searchInfo.reviewer" clearable placeholder="请选择" @clear="()=>{searchInfo.reviewer=undefined}">
              <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
            </el-select>
            </el-form-item>
        <el-form-item label="审核进度">
         <el-input v-model="searchInfo.progress" placeholder="搜索条件" />
        ...
        <el-form-item label="差错原因">
         <el-input v-model="searchInfo.errorReason" placeholder="搜索条件" />
        ...
        <el-form-item label="备注">
         <el-input v-model="searchInfo.note" placeholder="搜索条件" />
           ...
           <el-form-item label="本地账单ID" prop="localBillId">
            <el-select v-model="searchInfo.localBillId" clearable placeholder="请选择" @clear="()=>{searchInfo.localBillId=undefined}">
              <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
            </el-select>
            </el-form-item>
           <el-form-item label="第三方账单ID" prop="thirdPartyBillId">
            <el-select v-model="searchInfo.thirdPartyBillId" clearable placeholder="请选择" @clear="()=>{searchInfo.thirdPartyBillId=undefined}">
              <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
            </el-select>
            </el-form-item>
        <el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="交易员工ID" prop="tradingUserId" width="120">
            ...
            {{ filterDict(scope.row.tradingUserId,intOptions) }}
            </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="对账员ID" prop="reconciler" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.reconciler,intOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="审核员ID" prop="reviewer" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.reviewer,intOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="审核进度" prop="progress" width="120" />
        <el-table-column align="left" label="审核理由" prop="reason" width="120" />
        <el-table-column align="left" label="差错原因" prop="errorReason" width="120" />
        ...
        <el-table-column align="left" label="本地账单ID" prop="localBillId" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.localBillId,intOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="第三方账单ID" prop="thirdPartyBillId" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.thirdPartyBillId,intOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateErrorBillFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        <el-form-item label="交易员工ID:"  prop="tradingUserId" >
          <el-select v-model="formData.tradingUserId" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="对账员ID:"  prop="reconciler" >
          <el-select v-model="formData.reconciler" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="审核员ID:"  prop="reviewer" >
          <el-select v-model="formData.reviewer" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="审核进度:"  prop="progress" >
            <el-select v-model="formData.progress" placeholder="请选择" style="width:100%" :clearable="true" >
               <el-option v-for="item in ['成功','驳回','审核中','未审核']" :key="item" :label="item" :value="item" />
            </el-select>
        </el-form-item>
        ...
        <el-form-item label="本地账单ID:"  prop="localBillId" >
          <el-select v-model="formData.localBillId" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="第三方账单ID:"  prop="thirdPartyBillId" >
          <el-select v-model="formData.thirdPartyBillId" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
      </el-form>
...
const formData = ref({
        ...
        errorReason: '',
        note: '',
        localBillId: undefined,
        thirdPartyBillId: undefined,
        })
...
const getTableData = async() => {
  const table = await getErrorBillList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteErrorBillByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateErrorBillFunc = async(row) => {
    const res = await findErrorBill({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteErrorBillFunc = async (row) => {
    const res = await deleteErrorBill({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
