<template>
  ...
  <el-table :data="tableData">
      ...
      <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="请填写理由和最终确认的管理员">
       {{ "管理员ID:" }}<el-select  v-model="bill.adminId" placeholder="请选择审批的管理员" @change="getAdminName()" >
        <el-option el-option v-for="user in admin" :label="`${user.ID}`" :value="`${user.ID}`"></el-option>
        </el-select>
      {{ bill.admin }}
...
const getTableData = async() => {
  console.log(page)
  tableData.value = []
  const table = await getBillList({ page: page.value, pageSize: pageSize.value, keyword: userStore.userInfo.ID.toString(),keyType: "approverId" })
  if (table.code === 0) {
    let tempTotal = 0
    for (const index in table.data.list){
      const bill = table.data.list[index]
      console.log("billl",table.data.list[index])
      if(bill.type == 1 && bill.approvalState == 0){
...
const getAdminName = async()=>{
  ...
  console.log("admin###",admin.value[0])
  for (const index in admin.value){
    console.log(admin.value[index].ID)
    console.log( bill.value.adminId)

    if(admin.value[index].ID  == bill.value.adminId){
      console.log("admin###",admin.value[index]) 
      bill.value.admin = admin.value[index].userName
      return
    }
  }
}
