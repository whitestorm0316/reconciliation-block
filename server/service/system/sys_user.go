...
// 获取一类角色用户
func (userService *UserService) GetUsersByAuthorityId(AuthorityId int) ([]system.SysUser, error) {
	users := make([]system.SysUser, 0)
	if err := global.GVA_DB.Model(&system.SysUser{}).Where("authority_id=?", AuthorityId).Find(&users).Error; err != nil {
		return nil, err
	}
	return users, nil
}
